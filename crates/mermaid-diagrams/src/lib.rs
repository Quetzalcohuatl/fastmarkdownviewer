...
pub fn render_to_scene(
    ...
    match kind {
        ...
        DiagramKind::Flowchart => {
            ...
            // FMV local policy: avoid unbounded recursive dagre traversal.
            // This is a supported-size limit, not a general layout sandbox.
            if diagram.vertices.len() > 512
                || diagram.edges.len() > 1024
                || diagram.subgraphs.len() > 32
            {
                return Err(ParseError::new(
                    common::error::ParseErrorKind::ResourceLimit,
                    0..0,
                    input,
                ));
            }
...
mod fmv_regressions {
    ...
    #[test]
    fn fmv_rejects_unclosed_shapes_and_quotes() {
        for source in [
            "flowchart LR\nA[Unclosed",
            "flowchart LR\nA[\"Unclosed] --> B",
            "flowchart LR\nA[Unclosed --> B[Finish]",
            ...
            assert!(
                flowchart::parser::parse(source).is_err(),
                "accepted {source}"
            );
        }
        assert!(flowchart::parser::parse("flowchart LR\nA[\"A [quoted] label\"] --> B").is_ok());
    }

    #[test]
    fn fmv_outer_reference_preserves_inner_membership_and_bounds() {
        let source = "flowchart TD\nsubgraph App\nsubgraph Reader\nA[Open] --> B[Parse]\nend\nB --> C[Render]\nend\nC --> D[Screen]";
        let diagram = flowchart::parser::parse(source).unwrap();
        let reader = diagram.subgraphs.iter().find(|s| s.id == "Reader").unwrap();
        assert_eq!(reader.node_ids, ["A", "B"]);
        let app = diagram.subgraphs.iter().find(|s| s.id == "App").unwrap();
        assert!(!app.node_ids.iter().any(|n| n == "B"));
        let layout = flowchart::bridge::layout(&diagram);
        let group = layout.subgraphs.iter().find(|s| s.id == "Reader").unwrap();
        let node = layout.nodes.iter().find(|n| n.id == "B").unwrap();
        assert!(node.x - node.width / 2.0 >= group.x - group.width / 2.0 - 0.01);
        assert!(node.x + node.width / 2.0 <= group.x + group.width / 2.0 + 0.01);
        assert!(node.y - node.height / 2.0 >= group.y - group.height / 2.0 - 0.01);
        assert!(node.y + node.height / 2.0 <= group.y + group.height / 2.0 + 0.01);
    }

    #[test]
    fn fmv_mindmap_identifier_is_not_visible_text() {
        let diagram =
            mindmap::parser::parse("mindmap\n  root((Markdown viewer))\n    child[Reading]")
                .unwrap();
        assert_eq!(diagram.root.text, "Markdown viewer");
        assert_eq!(diagram.root.shape, mindmap::ir::MindmapShape::Circle);
        assert_eq!(diagram.root.children[0].text, "Reading");
    }

    #[test]
    fn fmv_rejects_large_graph_before_layout() {
        let mut source = String::from("flowchart TD\n");
        for n in 0..2000 {
            source.push_str(&format!("N{n} --> N{}\n", n + 1));
        }
        let error = render_to_scene(&source, &Theme::default()).unwrap_err();
        assert_eq!(error.kind, common::error::ParseErrorKind::ResourceLimit);
    }

    #[test]
    fn fmv_rejects_excessive_subgraph_recursion() {
        let mut source = String::from("flowchart TD\n");
        for n in 0..1000 {
            source.push_str(&format!("subgraph S{n}\n"));
        }
        source.push_str("A\n");
        source.push_str(&"end\n".repeat(1000));
        assert!(render_to_scene(&source, &Theme::default()).is_err());
    }
}
