...
8. `links` classifies every click before `platform` opens a browser or the current window loads a Markdown tab. Heading fragments resolve against the outline geometry.
...
Windows font fallback reads Segoe UI/Arial plus on-demand Gothic, Microsoft YaHei, Malgun Gothic, Nirmala UI, and Leelawadee UI from the Windows fonts directory, retaining bundled emoji. Large fonts are checked once per opened tab and installed once per context. Epaint already uses HarfRust shaping; these fallbacks improve glyph coverage without adding dependencies or full Unicode bidi paragraph layout.

The reading column is capped at 960 logical pixels. Tables estimate relative column widths from text lengths (capped per column), wrap individual cells, and own a horizontal scroll area. Nested blockquotes/definitions use the existing event renderer with balanced delimiters and saved line state. Inline styles restore a stack of surrounding styles, and selectable labels retain trailing whitespace between spans. Image failures use the existing loader error and retry/invalidation path.

The document viewport supports both scroll axes with solid, space-reserving bars inherited by tables and code. Prose width is tied to the visible viewport minus content padding even when horizontal scrolling offers unbounded layout space. Oversized blocks can expand the content bounds; table/code overflow stays within nested scroll areas. Search jumps clamp the parent horizontal target to the nested object's clip while its inner scroll area reveals the actual match.

Word wrap is a default-on session preference passed through CommonMarkOptions. It applies to prose, table cells, and code layout without modifying source text. Table rows use fixed column origins and the maximum measured cell height; no-wrap columns are sized from font measurements. Explicit Markdown block breaks remain active in both modes. Code frames use actual content height rather than the pre-layout scroll viewport height.
