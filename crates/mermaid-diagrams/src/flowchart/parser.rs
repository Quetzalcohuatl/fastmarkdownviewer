...
fn parse_subgraph(
    ...
    // Bound recursive subgraph parsing before descending further.
    if diagram.subgraphs.len() >= 32 {
...
fn parse_node_ref(
    ...
    // Once an opening delimiter is present, a failed label is an error, not
    // a bare-node reference. The old fallback swallowed consumed bad input.
    let shape_label = if input.starts_with(['[', '(', '{', '>']) {
        Some(parse_node_shape(input)?)
    } else {
        None
    };
    ...
    if let Some((shape, raw_label)) = shape_label {
    ...
    let already_grouped = diagram
        .subgraphs
        .iter()
        .any(|sg| sg.node_ids.contains(&id_str));
    if !already_grouped
        && let Some(sg_id) = subgraph_id
        && let Some(sg) = diagram.subgraphs.iter_mut().find(|s| s.id == sg_id)
