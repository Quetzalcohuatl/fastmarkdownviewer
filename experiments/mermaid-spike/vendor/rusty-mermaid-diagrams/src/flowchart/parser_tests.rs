...
#[test]
fn unclosed_bracket_in_node_shape_is_rejected() {
    // FMV requires errors instead of silently discarding malformed labels.
...
#[test]
fn unclosed_parenthesis_in_node_is_rejected() {
    // FMV requires errors instead of silently discarding malformed labels.
    assert!(parse("graph TD\n    A(text").is_err());
}

#[test]
fn unclosed_curly_in_node_is_rejected() {
    // FMV requires errors instead of silently discarding malformed labels.
    assert!(parse("graph TD\n    A{text").is_err());
}
