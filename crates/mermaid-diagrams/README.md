...
This production fork is published as `fmv-rusty-mermaid-diagrams` and used by the viewer through its guarded render entry point in an isolated helper process. The historical experiment retains its original copy. No upstream source cache was edited.
...
Run unit tests from the repository workspace:

```powershell
cargo test --locked -p fmv-rusty-mermaid-diagrams --lib
```
