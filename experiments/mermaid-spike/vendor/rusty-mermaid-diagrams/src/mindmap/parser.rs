...
fn parse_node_content(content: &str) -> MindmapNode {
    ...
    // Mermaid allows an ID before a shaped label: root((Visible label)).
    // Only strip a contiguous identifier; preserve ordinary prose with brackets.
    let content = if let Some(at) = content.find(['(', ')', '[', '{']) {
        let id = &content[..at];
        if !id.is_empty()
            && id
                .chars()
